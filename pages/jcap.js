...
//div.innerHTML = document.write("<p><input class='form-control' type=\"text\" id=\"" + jfldid + "\" name=\"" + jfldid + "\" class=\"" + jfldcls + "\" size=\"" +  jfldsz + "\"><\/p>");
//document.write("<p><img src=\"" + decodeURIComponent(imgdir) + imgid + ".jpg\" width=\"270\" height=\"80\" alt=\"\"><\/p>");
}
...
var valorDigitado = document.getElementById('jfldid').value;
if (valorDigitado == "") {
	$("#alertaDeRespostas").addClass("alert alert-warning");
	$("#alertaDeRespostas").text("O Código de verificação é um campo obrigatório.");
...
if (uword==cword[anum-1]) {
	return enviarCadastroCelular(inputNumeroCelular);
}

else {
	$("#alertaDeRespostas").addClass("alert alert-warning");
	$("#alertaDeRespostas").text("Erro: Código incorreto.");
	$("#alertaDeRespostas").show('fast').delay(3000).fadeOut(1000);
	$("#jfldid").focus();
	return false;
}
